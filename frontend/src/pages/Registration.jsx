...
const RegistrationPage = () => {
  ...
  const handleRegistration = async(e) => {
    e.preventDefault();
    try {
     const result = await axios.post(`${serverValue}`+ '/api/auth/registration',{
        name,email,password
     ...
     <header className="w-full p-2">
      ...
      <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-xl shadow-lg border border-gray-100">
        ...
        <div className="text-center">
          ...
          <h2 className="text-3xl font-semibold text-gray-900 mt-4">Registration </h2>
          <p className="text-gray-500 mt-2">Welcome to MagiCart, Place your order</p>
        </div>

        {/* Google Auth Button */}
        <button className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors duration-200 font-medium" onClick={googleRegistration}>
        ...
        <form className="space-y-4">
          ...
          <button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-lg shadow-md hover:shadow-lg transition-all transform active:scale-[0.98] mt-4" 
        ...
        {/* Footer Link */}
        <p className="text-center text-gray-600 mt-6 cursor-pointer" onClick={() => navigate('/login')}>
